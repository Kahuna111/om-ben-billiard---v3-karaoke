...
async function refreshData() {
  ...
  const tableSessions = allSessions.filter(
    (s) => s.targetType === "table" || !s.targetType,
  );

  // Stability check: only render if changes detected
  if (
    JSON.stringify(tables) !== JSON.stringify(newTables) ||
    JSON.stringify(activeSessions) !== JSON.stringify(tableSessions)
  ) {
    tables = newTables;
...
function renderCategoryTabs() {
  ...
  const descriptions = [
    ...new Set(tables.map((t) => t.description || "Standar")),
  ];
...
function renderTables() {
  ...
  const searchVal = document.getElementById("search-dashboard-input")
    ? document.getElementById("search-dashboard-input").value.toLowerCase()
    : "";
  ...
  const filtered = tables.filter((table) => {
    ...
    const matchesSearch =
      table.name.toLowerCase().includes(searchVal) ||
...
function openOrderModal(sessionId) {
  ...
  if (session.orders && session.orders.length > 0) {
    ...
    list.innerHTML =
      '<tr><td colspan="3" style="text-align:center; padding:1rem;">Belum ada pesanan</td></tr>';
  }
...
async function addOrderToSession() {
  ...
  const menuItem = menuItems.find(
    (m) =>
      `${m.name} - ${formatRupiah(m.price)}` === menuVal ||
      m.name.toLowerCase() === menuVal.toLowerCase(),
  );
  if (!menuItem) {
  ...
  if (qty > (menuItem.stock || 0)) {
    alert(
      `Maaf, stok ${menuItem.name} tidak mencukupi (Tersisa: ${menuItem.stock || 0}).`,
    );
    return;
  }
...
async function openStopModal(sessionId, tableId) {
  ...
  const ordersAmount = session.orders
  ...
  document.getElementById("print-receipt-btn").onclick = () =>
    printReceipt(lastStopTransaction);
...
rentalType.onchange = () => {
  durationGroup.style.display =
    rentalType.value === "duration" ? "block" : "none";
};

startForm.onsubmit = async (e) => {
  e.preventDefault();
  const memberInput = document.getElementById("member-id-input");
  const memberIdRaw = memberInput ? memberInput.value : "";
  const memberId = String(memberIdRaw || "").replace(/\s/g, "");
  ...
  const data = {
    ...
    durationMinutes:
      parseInt(document.getElementById("duration-minutes").value) || 0,
    targetType: "table",
    memberId: /^\d{8}$/.test(memberId) ? memberId : "",
  };
...
window.addEventListener("load", () => {
  ...
  if (autoId) {
    setTimeout(() => {
      ...
      document.getElementById("customer-name").value = decodeURIComponent(
        name || "",
      );
      if (memberId) {
        const memberInput = document.getElementById("member-id-input");
...
async function checkMemberStatus() {
  ...
  if (!memberInput || !memberFeedback) {
    console.error(
      "[Dashboard] memberInput or memberFeedback element is missing!",
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      "[Dashboard] Fetching from:",
      `${apiBaseUrl}/members/check/${val}`,
    );
    const res = await fetch(`${apiBaseUrl}/members/check/${val}`)
      .then((r) => r.json())
      .catch((err) => {
