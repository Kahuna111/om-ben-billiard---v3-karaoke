...
async function refreshData() {
  ...
  if (
    JSON.stringify(rooms) !== JSON.stringify(newRooms) ||
    JSON.stringify(activeSessions) !== JSON.stringify(roomSessions)
  ) {
    rooms = newRooms;
...
function renderCategoryTabs() {
  ...
  const descriptions = [
    ...new Set(rooms.map((r) => r.description || "Standar")),
  ];
...
startForm.onsubmit = async (e) => {
  ...
  const memberIdTrimmed = String(memberIdRaw || "").replace(/\s/g, "");
  const memberId = /^\d{8}$/.test(memberIdTrimmed) ? memberIdTrimmed : "";
...
async function checkMemberStatus() {
  ...
  if (!memberInput || !memberFeedback) {
    console.error(
      "[Karaoke] memberInput or memberFeedback element is missing!",
    );
    return;
  }
